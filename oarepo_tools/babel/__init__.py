...
try:
    ...
    click.secho("Babel is not installed in the current virtualenv. "
                "Please install it using \"pip install babel\" or "
                "add babel, polib and jinja2 to your dev depedencies.", fg="red")
    sys.exit(1)

try:
    import polib
except ImportError:
    click.secho("polib is not installed in the current virtualenv. "
...
try:
    ...
    click.secho("Jinja2 is not installed in the current virtualenv. "
...
def extract_babel_messages(base_dir, i18n_configuration, translations_dir):
    ...
    sources = [
        str(base_dir / x)
        for x in i18n_configuration["babel_source_paths"]
        if x.strip()
    ]

    translations_file = str(translations_dir / "messages.pot")

    click.secho(f"Extracting babel messages from {', '.join(sources)} -> {translations_file}")

    CommandLineInterface().run(
        ["pybabel", "extract", "-F", babel_ini_file, "-k", "lazy_gettext", "-o", translations_file, *sources]
    )
...
def update_babel_translations(base_dir, translations_dir):
    ...
    CommandLineInterface().run(
        ["pybabel", "update", "--no-fuzzy-matching", "-i", f"{translations_dir}/messages.pot", "-d", translations_dir]
    )
...
def merge_catalogues_from_translation_dir(source_translation_dir: Path, target_translation_dir: Path):
    for catalogue_file in source_translation_dir.glob("*/LC_MESSAGES/*.po"):
        merge_catalogues(catalogue_file, target_translation_dir / catalogue_file.relative_to(source_translation_dir))
